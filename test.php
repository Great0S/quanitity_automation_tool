...
/******************* ENTITY TYPES *****************/
     ...
     */
    protected function _validateEntityType($entityTypeId)
    {
        // Check if the entity type ID is numeric
        if (!is_numeric($entityTypeId)) {
            throw new InvalidArgumentException('Entity Type ID must be numeric.');
        }
    ...
    {
        // Validate the entity type before proceeding
        $entityTypeId = $this->getEntityTypeId($code);
        $this->_validateEntityType($entityTypeId);

        $this->updateTableRow('eav/entity_type',
            'entity_type_id', $entityTypeId, $field, $value
        );
        return $this;
    }
...
/******************* ATTRIBUTES *****************/
     ...
     * @return Mage_Eav_Model_Entity_Setup
     */
    protected function _updateAttribute($entityTypeId, $id, $field, $value, $sortOrder = null)
    {
        $this->_validateEntityType($entityTypeId);
        $mainTable = $this->getAttributeTable($entityTypeId, $id);  // Added $id as the second argument
        $additionalTable = $this->_getAttributeTypeTable($entityTypeId, $id);

        // Determine type and common fields
        $describe = $this->getConnection()->describeTable($mainTable);
        $mainFields = array_keys($describe);

        // Update fields in the main table
        ...
        if ($additionalTable) {
            $describe = $this->getConnection()->describeTable($additionalTable);
            $additionalFields = array_keys($describe);
        ...
        if ($sortOrder !== null) {
            $this->updateAttribute($entityTypeId, $id, 'sort_order', $sortOrder);
        }
    ...
    /**
     * Get attribute additional table name
     *
     * @param string|int $entityTypeId
     * @param string $id
     * @return string|false
     */
    protected function _getAttributeTypeTable($entityTypeId, $id)
    {
        $entityType = $this->getEntityType($entityTypeId);
        if (empty($entityType)) {
            return false;
        }

        $attributeCode = (is_numeric($id) ? $this->getAttributeCode($entityTypeId, $id) : $id);
        if (empty($attributeCode)) {
            return false;
        }

        $frontendInput = $this->getAttribute($entityTypeId, $attributeCode, 'frontend_input');
        ...
        switch ($frontendInput) {
            case 'select':
            case 'multiselect':
                return $this->getTable('eav/attribute_option');
            case 'text':
            case 'textarea':
                return $this->getTable('eav/attribute_text');
            case 'date':
            case 'datetime':
                return $this->getTable('eav/attribute_datetime');
            case 'decimal':
                return $this->getTable('eav/attribute_decimal');
            case 'int':
                return $this->getTable('eav/attribute_int');
            case 'varchar':
                return $this->getTable('eav/attribute_varchar');
            default:
                return false;
        }
    }
