...
def process_update_data():
    ...
    platform_updates, matching_values = get_platform_updates(data_lists, all_codes)
        ...
        for platform in platforms:
            ...
            if data[f'{platform}_data']:
                ...
                for item in data[f'{platform}_data']:
                    ...
                    if item['sku'] == code or re.search(code, item['sku']):
                        ...
                        if code in matching_ids:
                            ...
                            matching_ids[code].append(
                                {'platform': platform, 'id': item_id,  'qty': quantity})
                ...
                for product in products:
                    ...
                    if product['qty'] > lowest_val['qty']:
                        ...
                        matching_values.append(
                            {'sku': product['id'], 'qty1': product['qty'], 'qty2': lowest_val['qty'], 'value_difference': value_diff})

                        changed_values.append(
                            {'id': product['id'], 'sku': item_key, 'qty': str(lowest_val['qty']), 'platform': product['platform']})
            else:
