...
class WordPressAPI:
    ...
    def products_request(self):
        products_request = self.wcapi.get('products', params={"per_page": 100}).json()
        products = []
        count = 1
        ...
        if products_request:
            ...
            while products_request:
                ...
                count += 1
                products_request_raw = self.wcapi.get('products', params={"per_page": 100, "page": count})
    ...
    def get_wordpress_products(self, everyproduct: bool = False, local: bool = False):
        products = self.products_request()
    ...
    def update_wordpress_products(self, product_data: dict):
        stock_status = ''
        ...
        if isinstance(product_data, dict):
            ...
            update_request = self.wcapi.put(f"products/{product_data['id']}",
    ...
    def create_wordpress_products(self, product_data):
        if isinstance(product_data, dict):

            categories = self.get_products_categories()
            ...
            for item in product_data:
                ...
                create_product_request = self.wcapi.post(f"products",
                                                        product_data)
    ...
    def get_products_categories(self):
        ...
        request_json = self.wcapi.get("products/categories").json()
