...
class App:
    ...
    def retrieve_data(self, include_all_products: bool, use_local_data: bool, platforms: list) -> Tuple[dict, list]:
        ...
        returned_data = self.load_initial_data(include_all_products, platforms)
        
        # Access cached data
        if returned_data:
            cached_data = returned_data
        else:
    ...
    def process(self, data_dict: dict = None):
        ...
        if os.name == "nt":  # For Windows
            subprocess.run(shlex.split("cls")) # import subprocess, shlex
        else:  # For Linux and macOS
            subprocess.run(shlex.split("clear")) # import subprocess, shlex
